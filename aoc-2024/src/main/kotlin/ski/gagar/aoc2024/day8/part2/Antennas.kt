package ski.gagar.aoc2024.day8.part2
...
data class Coordinates(val row: Int, val col: Int) {
    ...
    fun oppositeTo(other: Coordinates): Sequence<Coordinates> {
        val delta = other - this
        return generateSequence(this) { it - delta }
    }
}
        ...
        for ((_, antennas) in byFrequency) {
            for (a1 in antennas) {
                for (a2 in antennas) {
                    ...
                    for (antinode in a1.oppositeTo(a2).takeWhile { isInside(it) }) {
