package ski.gagar.aoc2023.day5.part1
...
class Mapping(items: Sequence<MappingItem>) {
    ...
    operator fun get(src: Long): Long = map.floorEntry(src)?.value.get(src)
}
...
class Input(
    ...
    fun getLowestLocation() = seeds.minOfOrNull { mappings[it] }
}
...
object LocationsParser {
    ...
    private val SEEDS_SET = Parsers.sequence(
        TERMINALS.token(SEEDS), TERMINALS.token(COLON), INT_SET
    ) { _, _, ints -> ints}
